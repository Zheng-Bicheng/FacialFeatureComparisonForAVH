...
def resize_image(im_rgb, resize_short=112):
...
def create_headers(img_path_0, img_path_1):
    """
    ...
    img = cv2.imread(img_path_0)
    img = resize_norm_img(img)
    img_data = img.astype("float32")
    # Add the batch dimension, as we are expecting 4-dimensional input: NCHW.
    img_data = np.expand_dims(img_data, axis=0)
    create_header_file("input_0", "input_0", img_data, "include")
    # Create output header file
    output_data = np.zeros([512], np.float32)
    create_header_file("output_0", "output_0", output_data, "include")
    ...
    # Add the batch dimension, as we are expecting 4-dimensional input: NCHW.
    img_data = np.expand_dims(img_data, axis=0)
    create_header_file("input_1", "input_1", img_data, "include")
    # Create output header file
    output_data = np.zeros([512], np.float32)
    create_header_file("output_1", "output_1", output_data, "include")
